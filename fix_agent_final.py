"""Delete sloth-agent and recreate with tools enabled."""
...
# 1. Delete old sloth-agent
r = api("POST", "/api/v1/models/model/delete", token, {"id": "sloth-agent"})
print(f"Delete: {r}")
...
# 3. Create fresh with tools + prompt
...
change something, edit the file. When you are done, summarize what you did.
...
4. Explain what you are doing before each tool call
5. If a command fails, read the error and fix it - do not give up
6. Keep responses concise - show output, do not narrate it
...
if "error" not in result:
    ...
    print(f"  Tools: {result.get('meta', {}).get('tools', [])}")
    print(f"  Has prompt: {bool(result.get('params', {}).get('system_prompt'))}")
    print(f"  Prompt len: {len(result.get('params', {}).get('system_prompt', ''))}")
else:
    print(f"\nFAILED: {result}")
